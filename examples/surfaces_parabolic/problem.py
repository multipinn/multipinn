...
def sphere():
    ...
    inner_symbols = symbols(
        "u, u_x, u_y, u_z, u_t, u_xx, u_yy, u_zz, u_xy, u_xz, u_yz")
    basic_symbols = symbols("x, y, z, t")

    def surface(model, arg):
        u, u_x, u_y, u_z, u_t, u_xx, u_yy, u_zz, u_xy, u_xz, u_yz = inner_symbols(
            model, arg)
        x, y, z, t = basic_symbols(model, arg)
        eq1 = u_t - ((1-x**2)*u_xx + (1-y**2)*u_yy + (1-z**2)*u_zz - 2*x*y*u_xy - 2*x*z*u_xz - 2*y*z*u_yz - 2*x*u_x - 2*y*u_y - 2*z*u_z) \
            - ((1-x**2)*torch.sin(x)*torch.sin(y)*torch.sin(z)*(-1) * torch.sin(t) + (1-y**2)*torch.sin(x)*torch.sin(y)*torch.sin(z)*(-1) * torch.sin(t) + (1-z**2)*torch.sin(x)*torch.sin(y)*torch.sin(z)*(-1) * torch.sin(t) - 2*x*y*torch.cos(x)*torch.cos(y)*torch.sin(z) * torch.sin(t) - 2*x*z*torch.cos(x) * torch.sin(y)*torch.cos(z)
               * torch.sin(t) - 2*y*z*torch.sin(x)*torch.cos(y)*torch.cos(z) * torch.sin(t) - 2*x*torch.cos(x)*torch.sin(y) * torch.sin(z) * torch.sin(t) - 2*y*torch.sin(x)*torch.cos(y)*torch.sin(z) * torch.sin(t) - 2*z*torch.sin(x)*torch.sin(y)*torch.cos(z) * torch.sin(t) + torch.sin(x)*torch.sin(y)*torch.cos(z) * torch.cos(t))
        return [eq1]
