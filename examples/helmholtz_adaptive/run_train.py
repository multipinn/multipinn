...
def train(cfg: DictConfig):
    ...
    conditions, input_dim, output_dim = helmholtz_3D()
    ...
    generator_domain = AdaptiveGeneratorRAR_G(
        cfg.generator.domain_points,
        power_coeff=1,
        add_coeff=0,
        add_points=20000,
        n_points_up_bnd=1000000,
        density_rec_points_num=450000,
    )

    generator_bound = AdaptiveGeneratorRAR_G(
        n_points=cfg.generator.bound_points, density_rec_points_num=40000
    )
    ...
    callbacks = [
        ...
        curve.LearningRateCurve(cfg.paths.save_dir, cfg.visualization.save_period),
