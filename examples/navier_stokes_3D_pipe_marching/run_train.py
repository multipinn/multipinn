...
from examples.navier_stokes_3D_pipe_marching.problem import navier_stocks_3d_new_geom_problem_dist
...
def train(cfg: DictConfig):
    ...
    conditions, input_dim, output_dim, divide = navier_stocks_3d_new_geom_problem_dist(re=cfg.problem.re)
    ...
    generator_start = Generator(
        n_points=cfg.generator.start_points, sampler=cfg.generator.sampler
    )

    generator_domain.use_for(conditions[0])
    generator_start.use_for(conditions[1:3])
    generator_bound.use_for(conditions[3])
