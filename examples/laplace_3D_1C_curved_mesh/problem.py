...
def problem_3D1C_laplace_curved_mesh(mesh_file_path=None):
    input_dim = 3
    output_dim = 1
    ...
    domain = MeshArea(mesh_grid.get_face_by_id(2), mesh_grid.dim)
    bounds = [
        MeshArea(mesh_grid.get_face_by_id(idx), mesh_grid.dim) for idx in range(10, 22)
    ]
    ...
    pde = [Condition(inner, domain)] + [Condition(bc_all, bnd) for bnd in bounds]
